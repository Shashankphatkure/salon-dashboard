...
export async function getDailyStats(date) {
  try {
    ...
    // Calculate revenue
    const totalRevenue = transactions.reduce((sum, trans) => sum + (trans.amount || 0), 0);
    const totalCreditUsed = transactions.reduce((sum, trans) => sum + (trans.credit_used || 0), 0);
    const cashRevenue = totalRevenue - totalCreditUsed;

    // Calculate average service value
    const avgServiceValue = completedAppointments > 0 ? totalRevenue / completedAppointments : 0;
    ...
    staffPerformance.forEach(service => {
      ...
      const staffName = service.staff?.name || 'Unknown';
      ...
      staffStats[staffId].revenue += service.price || 0;
    });
...
export async function getUpcomingBirthdays() {
  ...
  const { data, error } = await supabase
    ...
    .lte('birthdate', nextWeek.toISOString().split('T')[0]);
...
export async function getUpcomingAnniversaries() {
  ...
  const { data, error } = await supabase
    ...
    .gte('anniversary', today.toISOString().split('T')[0])
    .lte('anniversary', nextWeek.toISOString().split('T')[0]);
