...
export async function createAppointment(appointment) {
  // Extract services if they exist in the data
  const services = appointment._services || [];
  delete appointment._services;

  // Handle customer data - remove any non-supported fields
  const { customer_name, customer_email, customer_phone, ...appointmentData } = appointment;

  console.log('Processing appointment data:', appointmentData);

  // Start a transaction to create an appointment and its services
  const { data, error } = await supabase
    .from('appointments')
  ...
  // If we have services to insert and the appointment was created
  if (services.length > 0 && data.id) {
    // Insert the appointment services
    await createAppointmentServices(data.id, services);
  }
  
  return data;
}

export async function createAppointmentServices(appointmentId, services) {
  // Format the services for insert
  ...
  console.log('Creating appointment services:', servicesToInsert);
