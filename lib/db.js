...
export async function createStaff(staff) {
  ...
  // Then add staff services if any
  if (staff.services && staff.services.length > 0) {
    const staffServices = staff.services.map(serviceId => ({
      staff_id: data.id,
...
export async function updateStaff(staffId, updates) {
  ...
  // Then handle services - first delete all existing services
  ...
  // Add updated services if any
  if (updates.services && updates.services.length > 0) {
    const staffServices = updates.services.map(serviceId => ({
      staff_id: staffId,
      service_id: serviceId
    }));
